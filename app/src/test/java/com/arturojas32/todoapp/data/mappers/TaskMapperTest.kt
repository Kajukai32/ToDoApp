...
class TaskMapperTest {
    ...
    private val fullRemoteTask = RemoteTask(
        id = 1,
        uid = "user123",
        remoteId = "remote_abc",
        title = "Buy groceries",
    ...
    @Test
    fun `RemoteTask toDomain maps all fields correctly`() {
        val task = fullRemoteTask.toDomain()
    ...
    @Test
    fun `RemoteTask toDomain maps nullable fields as null`() {
    ...
    @Test
    fun `Task toRemote maps all fields correctly`() {
        val remoteTask = fullTask.toRemote()

        assertEquals(1, remoteTask.id)
        assertEquals("user123", remoteTask.uid)
        assertEquals("remote_abc", remoteTask.remoteId)
        assertEquals("Buy groceries", remoteTask.title)
        assertEquals("Milk, eggs, bread", remoteTask.desc)
        assertEquals(true, remoteTask.done)
        assertEquals("15/01/2024", remoteTask.createdDate)
        assertEquals("20/01/2024", remoteTask.deadLine)
        assertEquals(1705276800000L, remoteTask.lastModified)
        assertEquals(true, remoteTask.synced)
        assertEquals(false, remoteTask.deleted)
    }

    // --- Round-trip RemoteTask <-> Task ---

    @Test
    fun `round trip RemoteTask to Task to RemoteTask preserves all fields`() {
        val result = fullRemoteTask.toDomain().toRemote()
        assertEquals(fullRemoteTask, result)
    }

    @Test
    fun `round trip Task to RemoteTask to Task preserves all fields`() {
        val result = fullTask.toRemote().toDomain()
        assertEquals(fullTask, result)
    }

    @Test
    fun `round trip RemoteTask with null fields preserves all fields`() {
        val remoteTask = RemoteTask(
            id = 3,
            uid = "user789",
            title = "Write tests",
        ...
        val result = remoteTask.toDomain().toRemote()
        assertEquals(remoteTask, result)
    }
}
